...
namespace StateNotifyByPop
{
    public static class LocalizationProvider
    {
        ...
        // 语言代码映射表
        private static readonly Dictionary<string, SystemLanguage> CodeToLanguage = new Dictionary<string, SystemLanguage>
        {
        ...
        /// 返回默认的硬编码文本，用于生成配置文件
        /// </summary>
        public static Dictionary<string, Dictionary<string, string>> GetDefaultLocalizations()
        {
        ...
        public static void InitTranslations()
        {
            ...
            var configLoc = ConfigManager.Config.localization;
            if (configLoc == null) return;

            foreach (var kv in configLoc)
            {
                string langCode = kv.Key;
                var map = kv.Value;

                if (CodeToLanguage.TryGetValue(langCode, out SystemLanguage sysLang))
                {
                    translations[sysLang] = map;
                }
                else
                {
                }
